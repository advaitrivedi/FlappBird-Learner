...
def trainNetwork(model,args):
    ...
    if args != "Train":
        t = 0
        temp = OBSERVE = 999999999    #We keep observe, never train
...
def plotGraph():
    ...
    with open('graph_p.pickle', 'rb') as f:
        g = pickle.load(f)
    ts = pickle.load(open('ts.pickle', 'rb'))
    #for i in range(3200,len(g['loss'])):
    #plt.ylim((0,250))
    plt.xlabel('Timestamp')
    plt.ylabel('Q_max')
    #plt.plot(i,g['loss'][i],'ro')
    # plt2.plot(i,g['Q_max'][i],'go')
    #plt3.plot(i,g['reward'][i],'ro')
    #plt.plot(range(3200,len(g['loss'])),g['loss'][3200:],'bo',range(3200,len(g['loss'])),g['loss'][3200:],'k')
    #.plot(range(3200,len(g['Q_max'])),g['Q_max'[3200:],'bo',range(3200,len(g['Q_max'])),g['Q_max'][3200:],'k')
    plt.plot(g['Q_max'])
